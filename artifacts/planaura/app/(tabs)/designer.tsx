...
function TBtn({ icon, active, danger, disabled, onPress, badge }: TBtnProps) {
  ...
  return (
    ...
    >
      ...
      {badge && (
        <View style={[styles.tBtnBadge, { backgroundColor: colors.destructive }]}>
          <Text style={styles.tBtnBadgeText}>{badge}</Text>
        </View>
      )}
    </ScalePress>
  );
      ...
      <View style={styles.overlay}>
        <View style={[styles.renameSheet, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.renameActions}>
            <ScalePress
              onPress={onClose}
              style={[styles.renameBtn, { backgroundColor: colors.mutedBg }]}
            >
              <Text style={[styles.renameBtnText, { color: colors.mutedForeground }]}>Cancel</Text>
            </ScalePress>
            <ScalePress
              onPress={() => onSave(val.trim() || currentName)}
...
export default function DesignerScreen() {
  ...
  const [show3DAlert, setShow3DAlert] = useState(false);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border, paddingTop: topPad + 6 }]}>
        ...
        <View style={styles.headerMeta}>
          {roomCount > 0 && (
            <Text style={[styles.metaText, { color: colors.mutedForeground }]}>{roomCount}rm · {store.currentPlan?.totalArea}sqft</Text>
          )}
          {vastuScore > 0 && (
      ...
      <View style={styles.canvasArea}>
        ...
        <View style={[styles.toolbar, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <TDivider />
          {/* History */}
          <TBtn icon="rotate-ccw" disabled={!canUndo} onPress={() => { if (canUndo) { store.undo(); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); } }} />
          <TBtn icon="rotate-cw" disabled={!canRedo} onPress={() => { if (canRedo) { store.redo(); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); } }} />
          <TDivider />
          {/* Delete */}
        ...
        <View style={[styles.zoomCluster, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <ScalePress onPress={() => handleZoom("in")} style={styles.zoomBtn} scale={0.9}>
            <Feather name="plus" size={14} color={colors.foreground} />
          </ScalePress>
          <Text style={[styles.zoomPct, { color: colors.mutedForeground }]}>{Math.round(store.zoom * 100)}%</Text>
          <ScalePress onPress={() => handleZoom("out")} style={styles.zoomBtn} scale={0.9}>
      ...
      {showPanel && (
        <View style={[styles.panel, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
          ...
          <View style={[styles.panelTabs, { borderBottomColor: colors.border }]}>
            ...
            <ScalePress onPress={() => setShowPanel(false)} style={styles.pClose} scale={0.9}>
...
const styles = StyleSheet.create({
  ...
  tBtnBadge: { position: "absolute", top: 4, right: 4, width: 8, height: 8, borderRadius: 4 },
  tBtnBadgeText: { color: "#fff", fontSize: 8 },
  ...
  roomBtn: {
    paddingHorizontal: 10, paddingVertical: 7, borderRadius: 9, borderWidth: 1.5,
  },
  ...
  viewToggle: {
    flexDirection: "row", borderRadius: 11, padding: 3, overflow: "hidden", gap: 2,
  },
