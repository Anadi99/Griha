...
import { View, Text, FlatList, StyleSheet, Platform, Alert, Animated } from "react-native";
...
function SkeletonCard() {
  ...
  return (
    <Animated.View style={[styles.skelCard, { backgroundColor: colors.card, borderColor: colors.border, opacity }]}>
      <View style={styles.skelTop}>
        <View style={[styles.skelIcon, { backgroundColor: colors.mutedBg }]} />
        <View style={{ flex: 1, gap: 8 }}>
          <View style={[styles.skelLine, { backgroundColor: colors.mutedBg, width: "60%" }]} />
          <View style={[styles.skelLine, { backgroundColor: colors.mutedBg, width: "40%", height: 10 }]} />
        </View>
      </View>
      <View style={{ flexDirection: "row", gap: 8 }}>
        <View style={[styles.skelChip, { backgroundColor: colors.mutedBg }]} />
        <View style={[styles.skelChip, { backgroundColor: colors.mutedBg, width: 80 }]} />
      </View>
    </Animated.View>
  );
}
...
function PlanCard({ item, index, onOpen, onDelete }: {
  ...
  useEffect(() => {
    Animated.spring(anim, {
      toValue: 1, tension: 120, friction: 10,
  ...
  return (
    <Animated.View style={{
      opacity: anim,
      transform: [{ translateY: anim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }, { scale: anim.interpolate({ inputRange: [0, 1], outputRange: [0.97, 1] }) }],
    }}>
      <ScalePress
        onPress={onOpen}
      ...
      >
        <View style={styles.cardTop}>
          ...
          <ScalePress onPress={onDelete} style={styles.deleteBtn} hitSlop={12} scale={0.88}>
...
export default function PlansScreen() {
  ...
  useEffect(() => {
    store.loadSavedPlans().then(() => setLoading(false));
  }, []);
  ...
  const handleNewPlan = () => {
    store.createNewPlan("New Floor Plan"); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    router.push("/(tabs)/designer");
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border, paddingTop: topPad + 8 }]}>
        ...
        <ScalePress onPress={handleNewPlan} style={[styles.newBtn, { backgroundColor: colors.primary }]}>
      ...
      {loading ? (
        ...
        <View style={styles.emptyState}>
          ...
          <ScalePress onPress={handleNewPlan} style={[styles.emptyBtn, { backgroundColor: colors.primary }]}>
            <Feather name="plus" size={16} color="#fff" />
...
const styles = StyleSheet.create({
  ...
  newBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, shadowColor: "#4F46E5", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 4 },
  newBtnText: { color: "#fff", fontSize: 14, fontWeight: "700" },
  list: { padding: 16, gap: 10 },

  skelCard: { borderRadius: 16, borderWidth: 1, padding: 16, gap: 12 },
  skelTop: { flexDirection: "row", gap: 12, alignItems: "center" },
  skelIcon: { width: 44, height: 44, borderRadius: 13 },
  skelLine: { height: 14, borderRadius: 7 },
  skelChip: { height: 26, width: 60, borderRadius: 13 },
  ...
  emptyBtn: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 24, paddingVertical: 14, borderRadius: 14, marginTop: 4, shadowColor: "#4F46E5", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.28, shadowRadius: 10, elevation: 5 },
